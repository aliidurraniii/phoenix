...
export type UsePatchServiceOptions<TData, TVariables> = {
  route: string;
  config?: AxiosRequestConfig;
} & Omit<UseMutationOptions<ApiResponse<TData>, ApiError, TVariables>, 'mutationFn'> &
  ApiHookOptions<ApiResponse<TData>>;

export const usePatchService = <TData extends object, TVariables = unknown>(
  options: UsePatchServiceOptions<TData, TVariables>,
) => {
  const { route, config, ...rest } = options;
  const { axiosClient } = useApiServiceContext();

  const mutationFn = async (payload: TVariables) => {
    const response = await axiosClient.patch<ApiResponse<TData>>(route, payload, {
      ...config,
    });
